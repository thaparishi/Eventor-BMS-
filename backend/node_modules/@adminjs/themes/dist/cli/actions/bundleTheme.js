...
*/
export const bundleTheme = async (input = './src/themes', themeId = undefined) => {
  if (themeId) {
    ...
    const themes = (await readdir(input, {
      withFileTypes: true
    })).filter(dirent => !dirent.isFile()).map(dirent => bundleComponents(input, dirent.name));
...
const bundleComponents = async (input, id) => {
  ...
  const components = componentsFiles.reduce((acc, file) => {
  ...
  } = await compile(components).then(bundle => bundle.generate({
    globals,
    format: 'iife',
...
const compile = async code => rollup({
  input: 'entry',
  plugins: [replace({
