...
*/
export const generateTheme = async (name, description, output = './src/themes') => {
  ...
  await writeFile(path.resolve(themeDir, 'overrides.ts'), [`import type { ThemeConfig } from 'adminjs';`, ``, `export const overrides: ThemeConfig['overrides'] = {};`, ``].join('\n'));
  await writeFile(path.resolve(themeDir, 'style.css'), '');
  await writeFile(path.resolve(themeDir, 'index.ts'), [`import type { ThemeConfig } from 'adminjs';`, `import { overrides } from './overrides.js';`, ``, `export const themeConfig: ThemeConfig = {`, `  id: '${themeId}',`, `  name: '${description || name}',`, `  overrides,`, `};`, ``].join('\n'));
